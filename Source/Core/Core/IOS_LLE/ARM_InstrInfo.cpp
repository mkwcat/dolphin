...
namespace IOS::LLE::ARMInstrInfo
{
...
#define A_IMPLEMENT_ALU_OP(x, k, a, c)                                                             \
  ...
  const u32 A_##x##_REG_ROR_IMM =                                                                  \
      A_RRXReadC | A_Write12 | c | A_##k | A_Read0 | ak(ak_##x##_REG_ROR_IMM);                     \
  const u32 A_##x##_REG_LSL_REG =                                                                  \
      A_Write12 | c | A_##k | A_Read0 | A_Read8 | ak(ak_##x##_REG_LSL_REG);                        \
  const u32 A_##x##_REG_LSR_REG =                                                                  \
      A_Write12 | c | A_##k | A_Read0 | A_Read8 | ak(ak_##x##_REG_LSR_REG);                        \
  const u32 A_##x##_REG_ASR_REG =                                                                  \
      A_Write12 | c | A_##k | A_Read0 | A_Read8 | ak(ak_##x##_REG_ASR_REG);                        \
  const u32 A_##x##_REG_ROR_REG =                                                                  \
      A_Write12 | c | A_##k | A_Read0 | A_Read8 | ak(ak_##x##_REG_ROR_REG);                        \
                                                                                                   \
  const u32 A_##x##_IMM_S = A_SetNZ | c | A_##a##_IMM | A_Write12 | A_##k | ak(ak_##x##_IMM_S);    \
  const u32 A_##x##_REG_LSL_IMM_S =                                                                \
      A_SetNZ | c | A_##a##_LSL_IMM | A_Write12 | A_##k | A_Read0 | ak(ak_##x##_REG_LSL_IMM_S);    \
  const u32 A_##x##_REG_LSR_IMM_S =                                                                \
      A_SetNZ | c | A_##a##_SHIFT_IMM | A_Write12 | A_##k | A_Read0 | ak(ak_##x##_REG_LSR_IMM_S);  \
  const u32 A_##x##_REG_ASR_IMM_S =                                                                \
      A_SetNZ | c | A_##a##_SHIFT_IMM | A_Write12 | A_##k | A_Read0 | ak(ak_##x##_REG_ASR_IMM_S);  \
  const u32 A_##x##_REG_ROR_IMM_S = A_RRXReadC | A_SetNZ | c | A_##a##_SHIFT_IMM | A_Write12 |     \
                                    A_##k | A_Read0 | ak(ak_##x##_REG_ROR_IMM_S);                  \
  const u32 A_##x##_REG_LSL_REG_S = A_SetNZ | c | A_##a##_SHIFT_REG | A_Write12 | A_##k |          \
                                    A_Read0 | A_Read8 | ak(ak_##x##_REG_LSL_REG_S);                \
  const u32 A_##x##_REG_LSR_REG_S = A_SetNZ | c | A_##a##_SHIFT_REG | A_Write12 | A_##k |          \
                                    A_Read0 | A_Read8 | ak(ak_##x##_REG_LSR_REG_S);                \
  const u32 A_##x##_REG_ASR_REG_S = A_SetNZ | c | A_##a##_SHIFT_REG | A_Write12 | A_##k |          \
                                    A_Read0 | A_Read8 | ak(ak_##x##_REG_ASR_REG_S);                \
  const u32 A_##x##_REG_ROR_REG_S = A_SetNZ | c | A_##a##_SHIFT_REG | A_Write12 | A_##k |          \
                                    A_Read0 | A_Read8 | ak(ak_##x##_REG_ROR_REG_S);
...
#define A_IMPLEMENT_ALU_TEST(x, a)                                                                 \
  ...
  const u32 A_##x##_REG_LSL_IMM =                                                                  \
      A_SetNZ | A_Read16 | A_##a##_LSL_IMM | A_Read0 | ak(ak_##x##_REG_LSL_IMM);                   \
  const u32 A_##x##_REG_LSR_IMM =                                                                  \
      A_SetNZ | A_Read16 | A_##a##_SHIFT_IMM | A_Read0 | ak(ak_##x##_REG_LSR_IMM);                 \
  const u32 A_##x##_REG_ASR_IMM =                                                                  \
      A_SetNZ | A_Read16 | A_##a##_SHIFT_IMM | A_Read0 | ak(ak_##x##_REG_ASR_IMM);                 \
  const u32 A_##x##_REG_ROR_IMM =                                                                  \
      A_RRXReadC | A_SetNZ | A_Read16 | A_##a##_SHIFT_IMM | A_Read0 | ak(ak_##x##_REG_ROR_IMM);    \
  const u32 A_##x##_REG_LSL_REG =                                                                  \
      A_SetNZ | A_Read16 | A_##a##_SHIFT_REG | A_Read0 | A_Read8 | ak(ak_##x##_REG_LSL_REG);       \
  const u32 A_##x##_REG_LSR_REG =                                                                  \
      A_SetNZ | A_Read16 | A_##a##_SHIFT_REG | A_Read0 | A_Read8 | ak(ak_##x##_REG_LSR_REG);       \
  const u32 A_##x##_REG_ASR_REG =                                                                  \
      A_SetNZ | A_Read16 | A_##a##_SHIFT_REG | A_Read0 | A_Read8 | ak(ak_##x##_REG_ASR_REG);       \
  const u32 A_##x##_REG_ROR_REG =                                                                  \
      A_SetNZ | A_Read16 | A_##a##_SHIFT_REG | A_Read0 | A_Read8 | ak(ak_##x##_REG_ROR_REG);
...
const u32 A_UMLAL =
    A_MulFlags | A_Write16 | A_Write12 | A_Read16 | A_Read12 | A_Read0 | A_Read8 | ak(ak_UMLAL);
const u32 A_SMULL = A_MulFlags | A_Write16 | A_Write12 | A_Read0 | A_Read8 | ak(ak_SMULL);
const u32 A_SMLAL =
    A_MulFlags | A_Write16 | A_Write12 | A_Read16 | A_Read12 | A_Read0 | A_Read8 | ak(ak_SMLAL);
...
const u32 A_SMLALxy =
    A_Write16 | A_Write12 | A_Read16 | A_Read12 | A_Read0 | A_Read8 | ak(ak_SMLALxy);
...
#define A_IMPLEMENT_WB_LDRSTR(x, k)                                                                \
  ...
  const u32 A_##x##_REG_ROR =                                                                      \
      A_##k | A_RRXReadC | A_Read16 | A_MemWriteback | A_Read0 | ak(ak_##x##_REG_ROR);             \
                                                                                                   \
  const u32 A_##x##_POST_IMM = A_##k | A_Read16 | A_Write16 | ak(ak_##x##_POST_IMM);               \
  const u32 A_##x##_POST_REG_LSL =                                                                 \
      A_##k | A_Read16 | A_Write16 | A_Read0 | ak(ak_##x##_POST_REG_LSL);                          \
  const u32 A_##x##_POST_REG_LSR =                                                                 \
      A_##k | A_Read16 | A_Write16 | A_Read0 | ak(ak_##x##_POST_REG_LSR);                          \
  const u32 A_##x##_POST_REG_ASR =                                                                 \
      A_##k | A_Read16 | A_Write16 | A_Read0 | ak(ak_##x##_POST_REG_ASR);                          \
  const u32 A_##x##_POST_REG_ROR =                                                                 \
      A_##k | A_RRXReadC | A_Read16 | A_Write16 | A_Read0 | ak(ak_##x##_POST_REG_ROR);
...
Info Decode(bool thumb, u32 num, u32 instr, bool literaloptimizations)
{
  const u8 FlagsReadPerCond[7] = {
      flag_Z, flag_C, flag_N, flag_V, flag_C | flag_Z, flag_N | flag_V, flag_Z | flag_N | flag_V};
  ...
  else
  {
    ...
    if (res.Kind == ak_LDM)
    {
      ...
      res.DstRegs |= set;
      // when the instruction is executed not in usermode a banked register in memory will be
      // written to but the unbanked register will still be allocated, so it is expected to carry
      // the proper value thus it is a source register
      if (instr & (1 << 22))
